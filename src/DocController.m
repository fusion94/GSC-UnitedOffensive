...
// this method adds the selected map + gametype to the rotation
{
    ...
    gameTypes = [@"xx DM TDM SD BEL RE HQ BAS CTF DOM" componentsSeparatedByString:@" "]; // xx is here to start at 1 instead of 0
...
- (void)netWorkDefaults
{
    ...
    [masterServer1 setStringValue:@"coduomaster.activision.com"];
    [masterServer2 setStringValue:@"master.gamespy.com"];
    [masterServer3 setStringValue:@"master0.gamespy.com"];
    [masterServer4 setStringValue:@"clanservers.net"];
    [masterServer5 setStringValue:@"coduoauthorize.activision.com"];
...
- (void)weaponDefaults
{
    ...
    [weaponPistols setState:NSOnState];
    [weaponSatchel setState:NSOnState];
    [weaponSmoke setState:NSOnState];
    [weaponGrenades setState:NSOnState];
    [weaponFlamethrower setState:NSOnState];
    [weaponArtillery setState:NSOnState];
    [weaponBazooka setState:NSOnState];
    [weaponMg30 setState:NSOnState];
    [weaponMg30BR setState:NSOnState];
    [weaponDp28 setState:NSOnState];
    [weaponSvt40 setState:NSOnState];
    [weaponMg34 setState:NSOnState];
    [weaponGewehr43 setState:NSOnState];
    
    [vehicleFlak88 setState:NSOnState];
    [vehicleSu152 setState:NSOnState];
    [vehicleElefant setState:NSOnState];
    [vehiclePanzerIV setState:NSOnState];
    [vehicleT34 setState:NSOnState];
    [vehicleSherman setState:NSOnState];
    [vehicleHorch setState:NSOnState];
    [vehicleGaz67B setState:NSOnState];
    [vehicleWillysJeep setState:NSOnState];
}

- (void)gameDefaults
{
    [bas_scoreLimit setIntValue:250];
    [bas_timeLimit setIntValue:20];
    [bas_roundLimit setIntValue:0];
    [bas_roundLength setIntValue:4];
    [bas_respawnWaveTime setIntValue:10];
    [bas_startRoundDelay setIntValue:5];
    [bas_endRoundDelay setIntValue:5];
    [bas_clearScoresEachRound setState:NSOnState];
    [ctf_scoreLimit setIntValue:250];
    [ctf_timeLimit setIntValue:20];
    [ctf_roundLimit setIntValue:0];
    [ctf_roundLength setIntValue:4];
    [ctf_startRoundDelay setIntValue:5];
    [ctf_endRoundDelay setIntValue:5];
    [ctf_clearScoresEachRound setState:NSOnState];
    [ctf_showOnCompass setState:NSOnState];
    [dom_scoreLimit setIntValue:250];
    [dom_timeLimit setIntValue:20];
    [dom_roundLimit setIntValue:0];
    [dom_roundLength setIntValue:4];
    [dom_respawnWaveTime setIntValue:10];
    [dom_startRoundDelay setIntValue:5];
    [dom_endRoundDelay setIntValue:5];
    [dom_clearScoresEachRound setState:NSOnState];
...
- (IBAction)rulesDefaults:(id)sender
{
    ...
    [allowVoteBanUser setState:NSOffState];
    [allowVoteKickUser setState:NSOnState];
    [allowVoteGameType setState:NSOnState];
    [allowVoteTypeMap setState:NSOnState];
    [allowVoteMap setState:NSOnState];
    [allowVoteNextMap setState:NSOnState];
    [allowVoteRestartMap setState:NSOnState];
...
// initialise all when the app launched
{
    ...
    c_masterServer3 = [[NSMutableString alloc] init];
    c_masterServer4 = [[NSMutableString alloc] init];
    c_masterServer5 = [[NSMutableString alloc] init];
...
// set the default map rotation
{
    ...
    gameTypes = [@"DM DOM HQ RET SD TDM BAS BEL CTF DOM DM" componentsSeparatedByString:@" "];
    mapList = [@"mp_arnhem mp_berlin mp_cassino mp_foy mp_italy mp_kharkov mp_kursk mp_ponyri mp_rhinevalley mp_sicily mp_uo_stanjel" componentsSeparatedByString:@" "];
...
// set the available map list for this gametype
{
    ...
    // 1=DM, 2=TDM, 3=SD, 4=BEL, 5=RE, 6=HQ, 7=BAS, 8=CTF, 9=DOM
    switch ([[gameType selectedItem] tag]) {
        ...
        case 8 :
        case 9 :
            mapList = [@"mp_arnhem mp_berlin mp_bocage mp_brecourt mp_carentan mp_cassino mp_chateau mp_dawnville mp_depot mp_foy mp_harbor mp_hurtgen mp_italy mp_kharkov mp_kursk mp_neuville mp_pavlov mp_ponyri mp_powcamp mp_railyard mp_rhinevalley mp_rocket mp_sicily mp_ship mp_uo_stanjel mp_tigertown" componentsSeparatedByString:@" "];
            break;
        case 3 :
            mapList = [@"mp_arnhem mp_berlin mp_bocage mp_brecourt mp_carentan mp_cassino mp_dawnville mp_depot mp_foy mp_harbor mp_hurtgen mp_italy mp_neuville mp_pavlov mp_powcamp mp_railyard mp_rocket mp_sicily mp_uo_stanjel mp_tigertown" componentsSeparatedByString:@" "];
            break;
        case 5 :
            mapList = [@"mp_arnhem mp_berlin mp_bocage mp_brecourt mp_carentan mp_cassino mp_chateau mp_dawnville mp_depot mp_foy mp_harbor mp_hurtgen mp_italy mp_neuville mp_pavlov mp_powcamp mp_railyard mp_rocket mp_sicily mp_ship mp_uo_stanjel mp_tigertown" componentsSeparatedByString:@" "];
            break;
        case 7 :
            mapList = [@"mp_bocage mp_brecourt mp_carentan mp_chateau mp_dawnville mp_depot mp_foy mp_harbor mp_hurtgen mp_italy mp_kursk mp_neuville mp_pavlov mp_ponyri mp_powcamp mp_railyard mp_rhinevalley mp_rocket mp_ship mp_tigertown" componentsSeparatedByString:@" "];
            break;
    }
    ...
    if ([[statusMessage stringValue] isEqualToString:GEN_RUNNING]) {
        ...
        if (fileExists && !isDir && ![selectedFile isEqualToString:@""]) { // if the file exists and is no directory, we're gonna open it.
            ...
            if (serverDict == nil) { // the file is damaged
                ...
                [masterServer3 setStringValue:[serverDict objectForKey:@"masterServer3"]];
                [masterServer4 setStringValue:[serverDict objectForKey:@"masterServer4"]];
                [masterServer5 setStringValue:[serverDict objectForKey:@"masterServer5"]];
                ...
                [setPlayerTimeOut setStringValue:[serverDict objectForKey:@"setPlayerTimeOut"]];
                ...
                [enableGameSpy setState:[[serverDict objectForKey:@"enableGameSpy"] isEqualToString:@"1"]];
                
                // game tab items
                [bas_scoreLimit setStringValue:[serverDict objectForKey:@"bas_scoreLimit"]];
                [bas_timeLimit setStringValue:[serverDict objectForKey:@"bas_timeLimit"]];
                [bas_roundLimit setStringValue:[serverDict objectForKey:@"bas_roundLimit"]];
                [bas_roundLength setStringValue:[serverDict objectForKey:@"bas_roundLength"]];
                [bas_respawnWaveTime setStringValue:[serverDict objectForKey:@"bas_respawnWaveTime"]];
                [bas_startRoundDelay setStringValue:[serverDict objectForKey:@"bas_startRoundDelay"]];
                [bas_endRoundDelay setStringValue:[serverDict objectForKey:@"bas_endRoundDelay"]];
                [bas_clearScoresEachRound setState:[[serverDict objectForKey:@"bas_clearScoresEachRound"] isEqualToString:@"1"]];
                [ctf_scoreLimit setStringValue:[serverDict objectForKey:@"ctf_scoreLimit"]];
                [ctf_timeLimit setStringValue:[serverDict objectForKey:@"ctf_timeLimit"]];
                [ctf_roundLimit setStringValue:[serverDict objectForKey:@"ctf_roundLimit"]];
                [ctf_roundLength setStringValue:[serverDict objectForKey:@"ctf_roundLength"]];
                [ctf_startRoundDelay setStringValue:[serverDict objectForKey:@"ctf_startRoundDelay"]];
                [ctf_endRoundDelay setStringValue:[serverDict objectForKey:@"ctf_endRoundDelay"]];
                [ctf_clearScoresEachRound setState:[[serverDict objectForKey:@"ctf_clearScoresEachRound"] isEqualToString:@"1"]];
                [ctf_showOnCompass setState:[[serverDict objectForKey:@"ctf_showOnCompass"] isEqualToString:@"1"]];
                [dom_scoreLimit setStringValue:[serverDict objectForKey:@"dom_scoreLimit"]];
                [dom_timeLimit setStringValue:[serverDict objectForKey:@"dom_timeLimit"]];
                [dom_roundLimit setStringValue:[serverDict objectForKey:@"dom_roundLimit"]];
                [dom_roundLength setStringValue:[serverDict objectForKey:@"dom_roundLength"]];
                [dom_respawnWaveTime setStringValue:[serverDict objectForKey:@"dom_respawnWaveTime"]];
                [dom_startRoundDelay setStringValue:[serverDict objectForKey:@"dom_startRoundDelay"]];
                [dom_endRoundDelay setStringValue:[serverDict objectForKey:@"dom_endRoundDelay"]];
                [dom_clearScoresEachRound setState:[[serverDict objectForKey:@"dom_clearScoresEachRound"] isEqualToString:@"1"]];
                ...
                [allowVoteBanUser setState:[[serverDict objectForKey:@"allowVoteBanUser"] isEqualToString:@"1"]];
                [allowVoteKickUser setState:[[serverDict objectForKey:@"allowVoteKickUser"] isEqualToString:@"1"]];
                [allowVoteGameType setState:[[serverDict objectForKey:@"allowVoteGameType"] isEqualToString:@"1"]];
                [allowVoteTypeMap setState:[[serverDict objectForKey:@"allowVoteTypeMap"] isEqualToString:@"1"]];
                [allowVoteMap setState:[[serverDict objectForKey:@"allowVoteMap"] isEqualToString:@"1"]];
                [allowVoteNextMap setState:[[serverDict objectForKey:@"allowVoteNextMap"] isEqualToString:@"1"]];
                [allowVoteRestartMap setState:[[serverDict objectForKey:@"allowVoteRestartMap"] isEqualToString:@"1"]];
                [self selectAllowVote:self];
                [enableShellShock setState:[[serverDict objectForKey:@"enableShellShock"] isEqualToString:@"1"]];
                [enableHealthDrop setState:[[serverDict objectForKey:@"enableHealthDrop"] isEqualToString:@"1"]];
                [enableBattleRank setState:[[serverDict objectForKey:@"enableBattleRank"] isEqualToString:@"1"]];
                ...
                [friendlyFire selectItemAtIndex:[[serverDict objectForKey:@"friendlyFire"] intValue]];
                ...
                [weaponSpingfieldBRIT setState:[[serverDict objectForKey:@"weaponSpingfield"] isEqualToString:@"1"]];
                ...
                [weaponPistols setState:[[serverDict objectForKey:@"weaponPistols"] isEqualToString:@"1"]];
                [weaponSatchel setState:[[serverDict objectForKey:@"weaponSatchel"] isEqualToString:@"1"]];
                [weaponSmoke setState:[[serverDict objectForKey:@"weaponSmoke"] isEqualToString:@"1"]];
                [weaponGrenades setState:[[serverDict objectForKey:@"weaponGrenades"] isEqualToString:@"1"]];
                [weaponFlamethrower setState:[[serverDict objectForKey:@"weaponFlamethrower"] isEqualToString:@"1"]];
                [weaponArtillery setState:[[serverDict objectForKey:@"weaponArtillery"] isEqualToString:@"1"]];
                [weaponBazooka setState:[[serverDict objectForKey:@"weaponBazooka"] isEqualToString:@"1"]];
                [weaponMg30 setState:[[serverDict objectForKey:@"weaponMg30"] isEqualToString:@"1"]];
                [weaponMg30BR setState:[[serverDict objectForKey:@"weaponMg30"] isEqualToString:@"1"]];
                [weaponDp28 setState:[[serverDict objectForKey:@"weaponDp28"] isEqualToString:@"1"]];
                [weaponSvt40 setState:[[serverDict objectForKey:@"weaponSvt40"] isEqualToString:@"1"]];
                [weaponMg34 setState:[[serverDict objectForKey:@"weaponMg34"] isEqualToString:@"1"]];
                [weaponGewehr43 setState:[[serverDict objectForKey:@"weaponGewehr43"] isEqualToString:@"1"]];
                
                [vehicleFlak88 setState:[[serverDict objectForKey:@"vehicleFlak88"] isEqualToString:@"1"]];
                [vehicleSu152 setState:[[serverDict objectForKey:@"vehicleSu152"] isEqualToString:@"1"]];
                [vehicleElefant setState:[[serverDict objectForKey:@"vehicleElefant"] isEqualToString:@"1"]];
                [vehiclePanzerIV setState:[[serverDict objectForKey:@"vehiclePanzerIV"] isEqualToString:@"1"]];
                [vehicleT34 setState:[[serverDict objectForKey:@"vehicleT34"] isEqualToString:@"1"]];
                [vehicleSherman setState:[[serverDict objectForKey:@"vehicleSherman"] isEqualToString:@"1"]];
                [vehicleHorch setState:[[serverDict objectForKey:@"vehicleHorch"] isEqualToString:@"1"]];
                [vehicleGaz67B setState:[[serverDict objectForKey:@"vehicleGaz67B"] isEqualToString:@"1"]];
                [vehicleWillysJeep setState:[[serverDict objectForKey:@"vehicleWillysJeep"] isEqualToString:@"1"]];
...
// this saves the current server config
{
    ...
    if (actionSelection != 0 ) { // user wants to save
        ...
        [configDict setObject:[masterServer3 stringValue] forKey:@"masterServer3"];
        [configDict setObject:[masterServer4 stringValue] forKey:@"masterServer4"];
        [configDict setObject:[masterServer5 stringValue] forKey:@"masterServer5"];
        ...
        [configDict setObject:[setPlayerTimeOut stringValue] forKey:@"setPlayerTimeOut"];
        ...
        [configDict setObject:[NSString stringWithFormat:@"%d", [enableGameSpy state]] forKey:@"enableGameSpy"];

        // game tab items
        [configDict setObject:[bas_scoreLimit stringValue] forKey:@"bas_scoreLimit"];
        [configDict setObject:[bas_timeLimit stringValue] forKey:@"bas_timeLimit"];
        [configDict setObject:[bas_roundLimit stringValue] forKey:@"bas_roundLimit"];
        [configDict setObject:[bas_roundLength stringValue] forKey:@"bas_roundLength"];
        [configDict setObject:[bas_respawnWaveTime stringValue] forKey:@"bas_respawnWaveTime"];
        [configDict setObject:[bas_startRoundDelay stringValue] forKey:@"bas_startRoundDelay"];
        [configDict setObject:[bas_endRoundDelay stringValue] forKey:@"bas_endRoundDelay"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [bas_clearScoresEachRound state]] forKey:@"bas_clearScoresEachRound"];
        [configDict setObject:[ctf_scoreLimit stringValue] forKey:@"ctf_scoreLimit"];
        [configDict setObject:[ctf_timeLimit stringValue] forKey:@"ctf_timeLimit"];
        [configDict setObject:[ctf_roundLimit stringValue] forKey:@"ctf_roundLimit"];
        [configDict setObject:[ctf_roundLength stringValue] forKey:@"ctf_roundLength"];
        [configDict setObject:[ctf_startRoundDelay stringValue] forKey:@"ctf_startRoundDelay"];
        [configDict setObject:[ctf_endRoundDelay stringValue] forKey:@"ctf_endRoundDelay"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [ctf_clearScoresEachRound state]] forKey:@"ctf_clearScoresEachRound"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [ctf_showOnCompass state]] forKey:@"ctf_showOnCompass"];
        [configDict setObject:[dom_scoreLimit stringValue] forKey:@"dom_scoreLimit"];
        [configDict setObject:[dom_timeLimit stringValue] forKey:@"dom_timeLimit"];
        [configDict setObject:[dom_roundLimit stringValue] forKey:@"dom_roundLimit"];
        [configDict setObject:[dom_roundLength stringValue] forKey:@"dom_roundLength"];
        [configDict setObject:[dom_respawnWaveTime stringValue] forKey:@"dom_respawnWaveTime"];
        [configDict setObject:[dom_startRoundDelay stringValue] forKey:@"dom_startRoundDelay"];
        [configDict setObject:[dom_endRoundDelay stringValue] forKey:@"dom_endRoundDelay"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [dom_clearScoresEachRound state]] forKey:@"dom_clearScoresEachRound"];
        ...
        [configDict setObject:[NSString stringWithFormat:@"%d", [allowVoteBanUser state]] forKey:@"allowVoteBanUser"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [allowVoteKickUser state]] forKey:@"allowVoteKickUser"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [allowVoteGameType state]] forKey:@"allowVoteGameType"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [allowVoteTypeMap state]] forKey:@"allowVoteTypeMap"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [allowVoteMap state]] forKey:@"allowVoteMap"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [allowVoteNextMap state]] forKey:@"allowVoteNextMap"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [allowVoteRestartMap state]] forKey:@"allowVoteRestartMap"];
        ...
        [configDict setObject:[NSString stringWithFormat:@"%d", [enableShellShock state]] forKey:@"enableShellShock"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [enableHealthDrop state]] forKey:@"enableHealthDrop"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [enableBattleRank state]] forKey:@"enableBattleRank"];
        ...
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponPistols state]] forKey:@"weaponPistols"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponSatchel state]] forKey:@"weaponSatchel"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponSmoke state]] forKey:@"weaponSmoke"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponGrenades state]] forKey:@"weaponGrenades"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponFlamethrower state]] forKey:@"weaponFlamethrower"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponArtillery state]] forKey:@"weaponArtillery"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponBazooka state]] forKey:@"weaponBazooka"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponMg30 state]] forKey:@"weaponMg30"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponDp28 state]] forKey:@"weaponDp28"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponSvt40 state]] forKey:@"weaponSvt40"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponMg34 state]] forKey:@"weaponMg34"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [weaponGewehr43 state]] forKey:@"weaponGewehr43"];
        
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleFlak88 state]] forKey:@"vehicleFlak88"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleSu152 state]] forKey:@"vehicleSu152"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleElefant state]] forKey:@"vehicleElefant"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehiclePanzerIV state]] forKey:@"vehiclePanzerIV"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleT34 state]] forKey:@"vehicleT34"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleSherman state]] forKey:@"vehicleSherman"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleHorch state]] forKey:@"vehicleHorch"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleGaz67B state]] forKey:@"vehicleGaz67B"];
        [configDict setObject:[NSString stringWithFormat:@"%d", [vehicleWillysJeep state]] forKey:@"vehicleWillysJeep"];
...
// check if document has been edited by user
{
    ...
    if (![c_masterServer3 isEqualToString:[masterServer3 stringValue]]) {serverEdited=YES;}
    if (![c_masterServer4 isEqualToString:[masterServer4 stringValue]]) {serverEdited=YES;}
    if (![c_masterServer5 isEqualToString:[masterServer5 stringValue]]) {serverEdited=YES;}
    ...
    if (c_setPlayerTimeOut != [setPlayerTimeOut intValue]) {serverEdited=YES;}
    ...
    // game tab items
    if (c_bas_scoreLimit != [bas_scoreLimit intValue]) {serverEdited=YES;}
    if (c_bas_timeLimit != [bas_timeLimit intValue]) {serverEdited=YES;}
    if (c_bas_roundLimit != [bas_roundLimit intValue]) {serverEdited=YES;}
    if (c_bas_roundLength != [bas_roundLength intValue]) {serverEdited=YES;}
    if (c_bas_respawnWaveTime != [bas_respawnWaveTime intValue]) {serverEdited=YES;}
    if (c_bas_startRoundDelay != [bas_startRoundDelay intValue]) {serverEdited=YES;}
    if (c_bas_endRoundDelay != [bas_endRoundDelay intValue]) {serverEdited=YES;}
    if (c_ctf_scoreLimit != [ctf_scoreLimit intValue]) {serverEdited=YES;}
    if (c_ctf_timeLimit != [ctf_timeLimit intValue]) {serverEdited=YES;}
    if (c_ctf_roundLimit != [ctf_roundLimit intValue]) {serverEdited=YES;}
    if (c_ctf_roundLength != [ctf_roundLength intValue]) {serverEdited=YES;}
    if (c_ctf_startRoundDelay != [ctf_startRoundDelay intValue]) {serverEdited=YES;}
    if (c_ctf_endRoundDelay != [ctf_endRoundDelay intValue]) {serverEdited=YES;}
    if (c_dom_scoreLimit != [dom_scoreLimit intValue]) {serverEdited=YES;}
    if (c_dom_timeLimit != [dom_timeLimit intValue]) {serverEdited=YES;}
    if (c_dom_roundLimit != [dom_roundLimit intValue]) {serverEdited=YES;}
    if (c_dom_roundLength != [dom_roundLength intValue]) {serverEdited=YES;}
    if (c_dom_respawnWaveTime != [dom_respawnWaveTime intValue]) {serverEdited=YES;}
    if (c_dom_startRoundDelay != [dom_startRoundDelay intValue]) {serverEdited=YES;}
    if (c_dom_endRoundDelay != [dom_endRoundDelay intValue]) {serverEdited=YES;}
...
// save current state to see if changed later on
{
    ...
    [c_masterServer3 setString:[masterServer3 stringValue]];
    [c_masterServer4 setString:[masterServer4 stringValue]];
    [c_masterServer5 setString:[masterServer5 stringValue]];
    ...
    c_setPlayerTimeOut = [setPlayerTimeOut intValue];
    ...
    // game tab items
    c_bas_scoreLimit = [bas_scoreLimit intValue];
    c_bas_timeLimit = [bas_timeLimit intValue];
    c_bas_roundLimit = [bas_roundLimit intValue];
    c_bas_roundLength = [bas_roundLength intValue];
    c_bas_respawnWaveTime = [bas_respawnWaveTime intValue];
    c_bas_startRoundDelay = [bas_startRoundDelay intValue];
    c_bas_endRoundDelay = [bas_endRoundDelay intValue];
    c_ctf_scoreLimit = [ctf_scoreLimit intValue];
    c_ctf_timeLimit = [ctf_timeLimit intValue];
    c_ctf_roundLimit = [ctf_roundLimit intValue];
    c_ctf_roundLength = [ctf_roundLength intValue];
    c_ctf_startRoundDelay = [ctf_startRoundDelay intValue];
    c_ctf_endRoundDelay = [ctf_endRoundDelay intValue];
    c_dom_scoreLimit = [dom_scoreLimit intValue];
    c_dom_timeLimit = [dom_timeLimit intValue];
    c_dom_roundLimit = [dom_roundLimit intValue];
    c_dom_roundLength = [dom_roundLength intValue];
    c_dom_respawnWaveTime = [dom_respawnWaveTime intValue];
    c_dom_startRoundDelay = [dom_startRoundDelay intValue];
    c_dom_endRoundDelay = [dom_endRoundDelay intValue];
...
- (IBAction)selectNoWeapons:(id)sender
{
    ...
    [weaponPistols setState:NSOffState];
    [weaponSatchel setState:NSOffState];
    [weaponSmoke setState:NSOffState];
    [weaponGrenades setState:NSOffState];
    [weaponFlamethrower setState:NSOffState];
    [weaponArtillery setState:NSOffState];
    [weaponBazooka setState:NSOffState];
    [weaponMg30 setState:NSOffState];
    [weaponMg30BR setState:NSOffState];
    [weaponDp28 setState:NSOffState];
    [weaponSvt40 setState:NSOffState];
    [weaponMg34 setState:NSOffState];
    [weaponGewehr43 setState:NSOffState];
...
// this is called by all weapons and checks for double action and after that, calls the weapon check
{
    switch ([sender tag]) {
        case 99:
        ...
        case 66:
            ...
            break;
        case 44:
            [weaponMg30BR setState:[weaponMg30 state]];
            break;
        case 88:
            [weaponMg30 setState:[weaponMg30BR state]];
            break;
    }
...
{
    BOOL oneUSASelected = ([weaponBar state] || [weaponM1Carbine state] || [weaponM1Garand state] || [weaponSpingfield state] || [weaponThompson state] || [weaponMg30 state]);
    BOOL oneBRITSelected = ([weaponBren state] || [weaponEnfield state] || [weaponSpingfieldBRIT state] || [weaponSten state] || [weaponMg30BR state]);
    BOOL oneGERMANSelected = ([weaponKar98k state] || [weaponKar98Sniper state] || [weaponMp40 state] || [weaponMp44 state] || [weaponMg34 state] || [weaponGewehr43 state]);
    BOOL oneRUSSelected = ([weaponNagant state] || [weaponNagantSniper state] || [weaponPpsh state] || [weaponDp28 state] || [weaponSvt40 state]);
...
{
    [vehicleFlak88 setState:NSOffState];
    [vehicleSu152 setState:NSOffState];
    [vehicleElefant setState:NSOffState];
    [vehiclePanzerIV setState:NSOffState];
    [vehicleT34 setState:NSOffState];
    [vehicleSherman setState:NSOffState];
    [vehicleHorch setState:NSOffState];
    [vehicleGaz67B setState:NSOffState];
    [vehicleWillysJeep setState:NSOffState];
}

- (IBAction)selectAllowVote:(id)sender
// switches the voting part on or off
{
    if ([allowVotes state]) {
        [allowVoteBanUser setEnabled:YES];
        [allowVoteKickUser setEnabled:YES];
        [allowVoteGameType setEnabled:YES];
        [allowVoteTypeMap setEnabled:YES];
        [allowVoteMap setEnabled:YES];
        [allowVoteNextMap setEnabled:YES];
        [allowVoteRestartMap setEnabled:YES];
    } else {
        [allowVoteBanUser setEnabled:NO];
        [allowVoteKickUser setEnabled:NO];
        [allowVoteGameType setEnabled:NO];
        [allowVoteTypeMap setEnabled:NO];
        [allowVoteMap setEnabled:NO];
        [allowVoteNextMap setEnabled:NO];
        [allowVoteRestartMap setEnabled:NO];
    }
